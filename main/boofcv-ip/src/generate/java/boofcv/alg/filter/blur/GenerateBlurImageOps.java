...
public class GenerateBlurImageOps  extends CodeGeneratorBase {
	...
	private void generateMeanExtended(AutoTypeImage type ) {
		String imageName = type.getSingleBandName();
		String letter = type.isInteger() ? "I" : type.getNumBits()==32?"F":"D";
		String suffix = type.getKernelType();
		...
		out.print(
				"\t/**\n" +
				"\t * Applies a mean box filter with extended image borders.\n" +
				"\t *\n" +
				"\t * @param input Input image.  Not modified.\n" +
				"\t * @param output (Optional) Storage for output image, Can be null.  Modified.\n" +
				"\t * @param radiusX Radius of the box blur function along the x-axis\n" +
				...
				"\t * @return Output blurred image.\n" +
				"\t */\n" +
				"\tpublic static "+imageName+" meanExt( "+imageName+" input, @Nullable "+imageName+" output, int radiusX, int radiusY,\n" +
				...
				"\t\tstorage = InputSanityCheck.checkDeclare(input,storage);\n" +
				"\n" +
				"\t\tboolean processed = BOverrideBlurImageOps.invokeNativeMeanExtended(input, output, radiusX, radiusY, binput, storage);\n" +
				"\n" +
				"\t\tif( processed )\n" +
				"\t\t\treturn output;\n" +
				"\n" +
	...
	void printPlanar() {
		...
		out.print(
				...
				"\t */\n" +
				"\tpublic static <T extends ImageGray<T>>\n" +
				"\tPlanar<T> meanExt(Planar<T> input, @Nullable Planar<T> output, int radiusX , int radiusY,\n" +
				...
				"\n" +
				"\t\tfor( int band = 0; band < input.getNumBands(); band++ ) {\n" +
				"\t\t\tGBlurImageOps.meanExt(input.getBand(band),output.getBand(band),radiusX,radiusY, binput, storage, workVert);\n" +
				"\t\t}\n" +
				"\t\treturn output;\n" +
				"\t}\n\n");
	}
