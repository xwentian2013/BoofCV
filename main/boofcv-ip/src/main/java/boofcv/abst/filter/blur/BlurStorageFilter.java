...
public class BlurStorageFilter<T extends ImageBase<T>> implements BlurFilter<T> {
	...
	public BlurStorageFilter( String functionName , ImageType<T> inputType,
		...
		if( functionName.equals("mean")) {
			...
			createStorage();
		} else if( functionName.equals("meanExt")) {
			operation = new MeanExtOperation();
			createStorage();
	...
	@Override
	public int getBorderX() { // TODO meanExt filter can now just process the inner image. This needs to be updated
		return 0;
	}
		...
		public void process(ImageBase input, ImageBase output) {
			if( border != null )
				throw new IllegalArgumentException(
						"Border has been set but will never be used. Must be a bug. Use meanExt() instead");
			GBlurImageOps.mean(input,output, radiusX,radiusY,storage,workArray);
		}
	}

	private class MeanExtOperation implements BlurOperation {
		@Override
		public void process(ImageBase input, ImageBase output) {
			GBlurImageOps.meanExt(input,output, radiusX,radiusY,(ImageBorder)border,storage,workArray);
		}
	}
