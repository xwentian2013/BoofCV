...
public class BOverrideBlurImageOps extends BOverrideClass {
	...
	public interface Mean<T extends ImageBase<T>> {
		...
		void processMeanExtended(T input, T output, int radiusX, int radiusY, @Nullable ImageBorder<T> border , T storage);
	}
	 ...
	 */
	public static <T extends ImageBase<T>>
	boolean invokeNativeMeanExtended(T input, T output, int radiusX, int radiusY, @Nullable ImageBorder<T> border , T storage) {
		boolean processed = false;
		if( BOverrideBlurImageOps.mean != null ) {
			try {
				BOverrideBlurImageOps.mean.processMeanExtended(input,output,radiusX,radiusY,border,storage);
				processed = true;
			} catch( RuntimeException ignore ) {}
		}
		return processed;
