...
public class GBlurImageOps {
	 ...
	 */
	public static <T extends ImageBase<T>>
	T meanExt(T input, @Nullable T output, int radiusX, int radiusY, @Nullable ImageBorder<T> border, @Nullable ImageBase storage , @Nullable WorkArrays workVert ) {
		if( input instanceof GrayU8) {
			return (T)BlurImageOps.meanExt((GrayU8) input, (GrayU8) output, radiusX, radiusY, (ImageBorder_S32)border, (GrayU8) storage, (IWorkArrays)workVert);
		} else if( input instanceof GrayU16) {
			return (T)BlurImageOps.meanExt((GrayU16)input,(GrayU16)output,radiusX, radiusY, (ImageBorder_S32)border,(GrayU16)storage, (IWorkArrays)workVert);
		} else if( input instanceof GrayF32) {
			return (T)BlurImageOps.meanExt((GrayF32)input,(GrayF32)output,radiusX, radiusY, (ImageBorder_F32)border,(GrayF32)storage, (FWorkArrays)workVert);
		} else if( input instanceof GrayF64) {
			return (T)BlurImageOps.meanExt((GrayF64)input,(GrayF64)output,radiusX, radiusY, (ImageBorder_F64)border, (GrayF64)storage, (DWorkArrays)workVert);
		} else if( input instanceof Planar) {
			return (T)BlurImageOps.meanExt((Planar)input,(Planar)output,radiusX, radiusY, (ImageBorder)border, (ImageGray)storage, workVert);
		} else  {
			throw new IllegalArgumentException("Unsupported image type");
		}
	}
